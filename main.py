...
users = [
    ...
    {'name': 'Petro', 'birthday': DT.datetime(1987, 7, 1)},
...
def get_weekday(user):
    name = user['name']
    current_birthday = get_this_year_birthday(user)
    new_date.update({name: current_birthday})
    return new_date[name].strftime("%A")
